...
while continue1=="y":
    ...
    for ind,(key,val) in enumerate(company_urls.items()):
        print(f"{ind+1}) {key}")
    user_input=input("Enter the company name to get the Interview Questions (Eg. microsoft,amazon,... etc):").lower()
...
# for key,val in company_urls.items():
#     scrape_interview_questions(val)
